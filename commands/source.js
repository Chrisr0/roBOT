...
module.exports = {
    ...
    execute(message, args) {
        ...
        request.post(sauceApiUrl, {form: options}, function (error, response, body) {
            ...
            if (result.data.creator || result.data.member_name || result.data.pawoo_user_display_name) embed.embed.fields.push({ "name": "Twórcy:", "value": JSON.stringify(result.data.creator || result.data.member_name || result.data.pawoo_user_display_name), "inline": "true" });
            if (result.data.characters) embed.embed.fields.push({ "name": "Postacie:", "value": JSON.stringify(result.data.characters), "inline": "true" });
            if (result.data.material) embed.embed.fields.push({ "name": "Materiał:", "value": JSON.stringify(result.data.material), "inline": "true" });
            if (result.data.part) embed.embed.fields.push({ "name": "Część:", "value": JSON.stringify(result.data.part), "inline": "true" });
            if (result.data.year) embed.embed.fields.push({ "name": "Rok:", "value": JSON.stringify(result.data.year), "inline": "true" });
